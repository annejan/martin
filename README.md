...
```bash
cargo run --release --bin splatgen -- list                  # sphere cube torus ring helix galaxy star wave knot mobius supershape lsystem fern menger shell tent pine flame rocket terrain moon mountains
...
camping set: `tent` (ridge tent), `pine` (a tiered conifer), `flame` (a campfire tongue — pairs with
`MARTIN_PARTICLES=embers`), `terrain` (rolling ground/hills so scenes aren't floating), `moon` (a grey
cratered moon). The space set: `rocket` (a cartoon rocket — white hull, red nose+fins, a hot exhaust
plume; nose points +Y so a `travel:` toward +Y reads as a launch). The rest are parametric solids
(sphere/cube/torus/ring/helix/galaxy/star/wave/knot/
mobius/supershape). All ~140k splats, deterministic (fixed seed → reproducible `.ply`).
