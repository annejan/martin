...
| `MARTIN_QUALITY` | — | One-word perf preset: **`low`** (count 120k · 960×540 · splat-scale 0.9 · sort-bits 16 — ~2.6× fps), **`med`** (250k · 1280×720), **`high`** (1920×1080). Caps the live window (`WIDTH`/`HEIGHT`) **and** the headless render (`MARTIN_RES`), so it governs `--record`/`--shot` too. Set-if-absent and applied *before* the show, so it also caps a show's own `budget`, but an explicit `MARTIN_MORPH_COUNT`/`_WIDTH`/`_RES`/… still wins. For running the demo on unknown party hardware. |
