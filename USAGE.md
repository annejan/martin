...
| `MARTIN_WIDTH` / `MARTIN_HEIGHT` | `1280` / `720` | Render **resolution** — the live **window** size *and* the headless offscreen image (record/shot). Lower = a big perf win on a weak GPU: the demo is largely **fill-rate bound**, so frame time scales ~linearly with pixel count (1080p ≈ 80 ms, 720p ≈ 45 ms, 540p ≈ 31 ms on the dev iGPU). |
...
| `MARTIN_QUALITY` | — | One-word perf preset: **`low`** (count 120k · 960×540 · splat-scale 0.9 — ~2.6× fps), **`med`** (250k · 1280×720), **`high`** (1920×1080). Set-if-absent and applied *before* the show, so it also caps a show's own `budget`, but an explicit `MARTIN_MORPH_COUNT`/`_WIDTH`/… still wins. For running the demo on unknown party hardware. |
