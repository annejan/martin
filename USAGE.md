...
| `MARTIN_BLOOM` | on | `=0` drops the HDR bloom (the glow-on-black look) — a quality/perf toggle for weak GPUs. **Note:** profiling shows bloom is cheap (~0.7 ms/frame), so this is rarely worth it; **count/resolution are the real levers**. The splats still render, just without the glow. |
