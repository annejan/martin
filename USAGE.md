...
| `MARTIN_PARTICLES` | off | **Additive particle layer** — glowing points drifting through the scene among the splats (the HDR bloom makes them glow), a second motion layer for instant "demo energy". The value picks the KIND: `embers` (warm sparks rising, default), `confetti` (tumbling coloured flakes falling), `sparks` (fast hot specks bursting from a core), `fireworks` (rise-then-bloom shells), `petals` (cherry-blossom rain — pink/white petals drifting + tumbling slowly down, aliases `blossom`/`sakura`), `meatballs` (real falling bitterbal.glb MESHES — "cloudy with a chance of meatballs", aliases `bitterballen`; adds its own lights since the scene has none). All **beat-reactive** — the kick scatters/pops them (embers stay calm to keep legacy bakes identical). `MARTIN_PARTICLE_COUNT` sets the number (default 200); `MARTIN_PARTICLE_AT=<secs\|@@anchor>` ramps the glow in from that time. Deterministic (each particle's transform is a pure function of its seed + the show clock + the clock-driven beat). RADV-safe (a real `StandardMaterial`, not a custom additive shader — which crashes the splat pipeline). |
| `MARTIN_PARTICLE_COUNT` | `200` | Number of embers when `MARTIN_PARTICLES` is set (clamped 1–5000). |
| `MARTIN_PARTICLE_AT` | `0` | `<secs\|@@anchor>` — ramp the particle glow in starting at this show-time (e.g. embers appear *with* the campfire on the drop). |
| `MARTIN_PARTICLE_OUT` | — | `<secs\|@@anchor>` — fade the glow back **out** at this time (e.g. embers leave *with* the fire when the camp clears), so they don't linger over an empty scene. |
| `MARTIN_PARTICLE_BACK` | — | `<secs\|@@anchor>` — fade the glow **in again** after an `_OUT` (e.g. embers return when the fire re-ignites for the party). `_AT` → `_OUT` → `_BACK` lets the sparks follow a fire with two lives. |
| `MARTIN_PARTICLE_ORIGIN` | scene centre | `x,y,z` — re-centre the particle field (e.g. on the campfire) so sparks rise *from* a point instead of filling the whole scene. Pairs with `_SPREAD`. |
| `MARTIN_PARTICLE_SPREAD` | wide box | `s` (all axes) or `x,y,z` — scale the particle box per axis around `_ORIGIN` (e.g. `1.5,0.7,1.5` = a wide, shorter campfire spread). |
...
| `MARTIN_POST` | off | **Post-processing** over the whole rendered frame, composable with `+`: `chroma` (RGB channel-split shearing red/cyan on the kick), `grain` (film grain), `vignette` (darkened corners), and the **`cine`** preset = all three for a cinematic look. Combine (`chroma+vignette`) and scale with `:<strength>` (`cine:1.2`). Runs in the camera's render graph after tonemapping, so it covers the window, the live `MARTIN_SERVE` view, and headless recordings alike. Deterministic/record-safe — chroma rides the clock-driven `kick`, grain is a pure hash of pixel + the clock-driven time. Default-off; splat geometry is untouched. See [Post-processing](#post-processing). |
...
| `MARTIN_MAX_DEMO_SECS` | `600` | Cap on a score's rendered length (seconds). The synth sizes its buffers from this, so it's bounded by default (10 min — far past any demo). Raise it for a **long-form one-off** (a full ~18-min Echoes: `MARTIN_MAX_DEMO_SECS=1200`); clamped to 2400 (~40 min, ~7 GB of buffers). |
...
| `MARTIN_ADDITIVE` | `0` | `=1` renders splats with an **additive/emissive** blend (light *accumulates*, `One+One`) instead of the default alpha-over. Overlapping translucent gaussians then **glow** on black — the nebula/neon demoscene look — rather than alpha-saturating to a solid opaque blob. **For glow content on a dark background** (there's no occlusion — everything shows through); solid captures and shows with a bright/coloured backdrop should stay `0` (additive blows those out). Also a `.show [settings] additive = 1`. Baked into the renderer fork (`CloudSettings.additive`, §10 — see DESIGN.md § the fork's shader edits). |
...
| `MARTIN_CULL` | `0` | `=1` re-enables Bevy's **frustum culling** on the splat clouds. Off by default on purpose: the renderer depth-sorts **per cloud**, so culling pops whole clouds in/out at the screen edge during fast camera moves — no-cull is the correct look. Turn it on only for a perf benchmark where you want to measure the culled ceiling. Also a `.show [settings] cull = 1`. |
| `MARTIN_NORMALIZE` | `1` | `=0` skips the per-part **centroid + p90 re-normalization** of reel parts, keeping every part's authored coordinates as-is. For content prepared in one **shared world frame** — e.g. `pipeline/nl_city_splat.py` route *segments*, where a mid-flight morph swaps segment *k* for *k+1* in place and per-part re-fitting would yank each segment back to the origin and break the seam. Leave `1` for normal shows (parts vary wildly in raw scale). Also a `.show [settings] normalize = 0`. |
| `MARTIN_QUALITY` | — | One-word perf preset, weak→strong: **`potato`** (640×360 · count ×0.4 · disks 0.7 · sort16 — the party-HW floor, ~60–90 fps even on the dense PonyCamp climax), **`low`** (854×480 · count ×0.5 · disks 0.8 · sort16 — ~60 fps-class, better quality), **`med`** (1280×720), **`high`** (1920×1080). Each caps the live window (`WIDTH`/`HEIGHT`) **and** the headless render (`MARTIN_RES`), so it governs `--record`/`--shot` too. Set-if-absent (an explicit `MARTIN_MORPH_COUNT`/`_RES`/… still wins). `potato`/`low` also set `MARTIN_COUNT_SCALE`, so they thin a **count-bound compose stage** (a diorama whose props pin their own `count:`), not just a reel. For running the demo on unknown party hardware. |
| `MARTIN_COUNT_SCALE` | `1.0` | Global **density multiplier** on every resolved gaussian count — the reel part `budget` **and** every compose object's `count:`/default. The one knob that thins a whole scene even past an explicit `budget=`/`count:` that `MARTIN_MORPH_COUNT` can't reach (so `MARTIN_QUALITY` can scale a count-bound compose stage). `0.5` ≈ half the splats everywhere; `1.0` = unchanged. Applied at build → record-safe. |
...
| `MARTIN_SPLAT_WARN` | `2000000` | Soft cap for the **GPU-OOM heads-up**. At build martin estimates a scene's PEAK resident gaussians and checks it against this (the dev iGPU OOMs ~2.5M). In a live/windowed run it logs a `WARN` and plays on; in **`--record`** it **refuses to start** (exit 1, before any frames) — a long unattended dump over the line would OOM the GPU mid-render (a silent truncated clip), so it fails fast + loud instead. Raise it if you've verified the hardware handles more. |
| `MARTIN_FPS` | off | `=1` logs smoothed FPS / frame-time + timeline clock every ~0.5 s (the **`I`** key toggles it live + logs a snapshot). |
| `MARTIN_FPS_OVERLAY` / `[settings] fps_overlay` | off | On-screen **FPS HUD** — a corner readout of smoothed FPS + frame-time + splat budget + clock, drawn over the live window (the engine's own perf counter, no external tool). `=1` starts it shown; the **`I`** key toggles it (alongside the console metric above). **Live windows only** — never drawn into a `--record` video or a `--shot`, so it can't bake into a deliverable. |
...
| `MARTIN_POST=` | effect |
|---|---|
| `chroma` (or `chroma:<strength>`) | **RGB channel-split** whose magnitude rides the kick — the frame shears red/cyan on every drum hit, snapping back between hits. `chroma:1.5` punches harder. Aliases: `rgb`/`split`. |
| `grain` (or `film`) | **Film grain** — a per-pixel hash dither, animated by the clock (deterministic). |
| `vignette` (or `vig`) | **Vignette** — darkened frame corners. |
| `cine` | **Preset** = `chroma` + `grain` + `vignette` at tuned strengths (chroma 0.7 · grain 0.06 · vignette 0.32) for a cinematic look. |
| `a+b[:<strength>]` | **Compose** any of the above with `+` (e.g. `chroma+vignette`); a trailing `:<f>` scales the whole stack (`cine:1.2`). |
