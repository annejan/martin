...
martin is a standalone Bevy 0.19 + `bevy_gaussian_splatting` 8.0.2 (our fork, a git
...
> so §11 is now a docs-only entry. §10 (additive blend) likewise went upstream as #238 (in 8.0.2),
> so it too is docs-only now. The sort speedup (§3) likewise went
