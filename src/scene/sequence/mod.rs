//! The one morph-timeline engine: a show is a list of `Shot`s that each assemble in from a
//! source cloud (ball/fade/explode/… or a per-particle shader transition) and then hold,
//! morphing into the next. Drives a single `GaussianInterpolate` entity retargeted per shot.
//!
//! Split across submodules:
//! - [`model`] — data types (`Shot`/`Sequence`/`BuiltShot`/`SeqState`/`FlashStrength`) + the pure
//!   cue-timeline functions (`active_shot`/`shot_starts`/`show_end`).
...
//! - [`director`] — `shot_director`: drive the morph per frame.
